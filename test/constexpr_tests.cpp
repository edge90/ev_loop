...
{
  STATIC_REQUIRE(ev_loop::detail::index_of_v<int, int, float, double> == 0);
  STATIC_REQUIRE(ev_loop::detail::index_of_v<float, int, float, double> == 1);
  STATIC_REQUIRE(ev_loop::detail::index_of_v<double, int, float, double> == 2);
}

TEST_CASE("const_max computes maximum at compile time", "[constexpr]")
{
  STATIC_REQUIRE(ev_loop::detail::const_max<1, 2, 3>() == 3);
  STATIC_REQUIRE(ev_loop::detail::const_max<3, 2, 1>() == 3);
  STATIC_REQUIRE(ev_loop::detail::const_max<5>() == 5);
  STATIC_REQUIRE(ev_loop::detail::const_max<1, 100, 50>() == 100);
}

TEST_CASE("tag_type_t selects smallest sufficient type", "[constexpr]")
{
  // uint8_t range: 0-254 (255 reserved for uninitialized)
  STATIC_REQUIRE(sizeof(ev_loop::detail::tag_type_t<10>) == 1);
  STATIC_REQUIRE(sizeof(ev_loop::detail::tag_type_t<200>) == 1);
  STATIC_REQUIRE(sizeof(ev_loop::detail::tag_type_t<254>) == 1);

  // uint16_t range: 255-65534
  STATIC_REQUIRE(sizeof(ev_loop::detail::tag_type_t<255>) == 2);
  STATIC_REQUIRE(sizeof(ev_loop::detail::tag_type_t<300>) == 2);
  STATIC_REQUIRE(sizeof(ev_loop::detail::tag_type_t<65534>) == 2);

  // uint32_t range: 65535 to uint32_max-1
  STATIC_REQUIRE(sizeof(ev_loop::detail::tag_type_t<65535>) == 4);
  STATIC_REQUIRE(sizeof(ev_loop::detail::tag_type_t<100000>) == 4);
  STATIC_REQUIRE(sizeof(ev_loop::detail::tag_type_t<4294967294UL>) == 4); // max valid value
...
{
  STATIC_REQUIRE(noexcept(std::declval<ev_loop::detail::TaggedEvent<int>&>().index()));
  STATIC_REQUIRE(noexcept(std::declval<const ev_loop::detail::TaggedEvent<int>&>().index()));
}
...
TEST_CASE("spsc::Queue::is_stopped is noexcept", "[constexpr]")
{
  STATIC_REQUIRE(noexcept(std::declval<const ev_loop::detail::spsc::Queue<int>&>().is_stopped()));
}

TEST_CASE("mpsc::Queue::is_stopped is noexcept", "[constexpr]")
{
...
TEST_CASE("Ref qualified functions are callable on lvalues", "[constexpr]")
{
  ...
  STATIC_REQUIRE(requires(ev_loop::detail::RingBuffer<int>& buf) { buf.empty(); });
  STATIC_REQUIRE(requires(ev_loop::detail::RingBuffer<int>& buf) { buf.size(); });
...
{
  using ConsumerTaggedEvent = ev_loop::detail::to_tagged_event_t<ev_loop::detail::get_receives_t<OwnThreadConsumerB>>;
  ...
  SECTION("Single external emitter selects SPSC queue")
  {
    ...
    STATIC_REQUIRE(
      std::is_same_v<Loop::queue_type_for<OwnThreadConsumerB>, ev_loop::detail::spsc::Queue<ConsumerTaggedEvent>>);
  }
  ...
  SECTION("SameThread + external selects MPSC queue")
  {
    ...
    STATIC_REQUIRE(
      std::is_same_v<Loop::queue_type_for<OwnThreadConsumerB>, ev_loop::detail::mpsc::Queue<ConsumerTaggedEvent>>);
  }
}
